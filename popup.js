...
// Chromium history.search has no pagination; use a high explicit cap instead of the undocumented zero-results cap behavior.
const ALL_HISTORY_MAX_RESULTS = 1_000_000;
...
const I18N = {
  en: {
    ...
    formatLabel: "Format",
    modeLabel: "Mode",
    modeVisits: "Current history entries",
  ...
  ru: {
    ...
    formatLabel: "Формат",
    modeLabel: "Режим",
    modeVisits: "Текущие записи истории",
  ...
  es: {
    ...
    formatLabel: "Formato",
    modeLabel: "Modo",
    modeVisits: "Entradas actuales del historial",
    ...
    privacyPermissions: "Los permisos se limitan al historial y al almacenamiento local.",
  ...
  de: {
    ...
    formatLabel: "Format",
    modeLabel: "Modus",
    modeVisits: "Aktuelle Verlaufseintraege",
...
I18N.fr = {
  ...
  formatLabel: "Format",
  modeLabel: "Mode",
  modeVisits: "Entrees actuelles de l'historique",
  ...
  privacyPermissions: "Les permissions sont limitees a l'historique et aux parametres locaux.",
...
I18N.pt = {
  ...
  formatLabel: "Formato",
  modeLabel: "Modo",
  modeVisits: "Entradas atuais do historico",
  modeDomains: "Agrupar por dominio",
  sourcesTitle: "Fontes",
...
I18N.uk = {
  ...
  formatLabel: "Формат",
  modeLabel: "Режим",
  modeVisits: "Поточні записи історії",
  ...
  privacyPermissions: "Дозволи обмежені історією та локальним сховищем налаштувань.",
...
async function loadFilteredRecords(options, job) {
  const records = [];
  let browserRecords = [];
  const needsCurrentHistoryBinding = options.sources.browser && (options.sources.archive || options.sources.takeout);

  if (options.sources.browser || needsCurrentHistoryBinding) {
    browserRecords = await readBrowserHistory(options, job);
  }
  ...
  if (options.sources.archive || options.sources.takeout) {
    ...
    let storedRecords = await readStoredRecords(options, sources);

    if (needsCurrentHistoryBinding) {
      const { records: currentBoundRecords, staleArchiveIds } = bindStoredRecordsToCurrentHistory(storedRecords, browserRecords);
      storedRecords = currentBoundRecords;
      if (staleArchiveIds.length > 0) {
        await deleteRecordsByIds(staleArchiveIds);
        void refreshArchiveStatus();
      }
    }
...
function bindStoredRecordsToCurrentHistory(storedRecords, browserRecords) {
  // Browser exports must not resurrect visits from local archive/import caches after the user deletes them.
  const currentVisitKeys = createCurrentVisitKeySet(browserRecords);
  const staleArchiveIds = [];
  const currentBoundRecords = storedRecords.filter((record) => {
    if (currentVisitKeys.has(createCurrentVisitKey(record))) {
      return true;
    }

    if (record.source === "archive" && record.id) {
      staleArchiveIds.push(record.id);
    }
    return false;
  });

  return { records: currentBoundRecords, staleArchiveIds };
}

function createCurrentVisitKeySet(records) {
  return new Set(records.map((record) => createCurrentVisitKey(record)));
}

function createCurrentVisitKey(record) {
  return `${Math.round(Number(record.visitTime) || 0)}:${record.url}`;
}
...
async function readBrowserHistory(options, job) {
  ...
  for (let index = 0; index < pages.length; index += batchSize) {
    ...
    const batchRecords = await Promise.all(batch.map((page) => readCurrentHistoryEntry(page, options)));
...
async function readCurrentHistoryEntry(page, options) {
  if (!page.url || typeof page.lastVisitTime !== "number") {
    return [];
  }

  const visitTime = page.lastVisitTime;
  if (visitTime < options.startTime || visitTime > options.endTime) {
    return [];
  }

  const title = normalizeText(page.title) || page.url;
  const domain = extractDomain(page.url, options.locale);
  const currentVisit = await readCurrentVisitMetadata(page.url, visitTime);
  ...
  return [createRecord({
    ...
    transition: currentVisit?.transition || "",
    referringVisitId: currentVisit?.referringVisitId || ""
  }, options.locale)];
}
...
function buildCsvBlob(rows, options) {
  ...
  const csvRows = [
    ...
    ...rows.map((row) => columns.map((column) => sanitizeCsvCell(columnValue(column, row, options.locale))))
  ];
